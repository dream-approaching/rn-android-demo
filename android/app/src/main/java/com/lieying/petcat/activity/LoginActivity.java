...
import com.lieying.petcat.subscribe.ControlSubscriber;
...
import com.lieying.petcat.widget.CountTimer;
...
public class LoginActivity extends BaseActivity implements  ButtonWaiting.OnWaitingListener<String> , TextWatcher, CountTimer.OnTimerListener {
    ...
    public void initListener() {
        ViewUtils.OnKeyBoardChangeListener onKeyBoardChangeListener = new ViewUtils.OnKeyBoardChangeListener() {
            @Override
        ...
        findViewById(R.id.tv_login_procy).setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        mOnGlobalLayoutListener = ViewUtils
                .getKeyboardListener(getWindow().getDecorView(), mLlLogin , onKeyBoardChangeListener);
    ...
    public void onStartTimer() {
        getVerifyCode( mEtPhone.getText().toString() );
    }
...
//                }onStartTimer
    ...
    private void getVerifyCode(String phone){
        ...
        RetrofitUtils.getInstance(mContext).sendRequset(new Function<String, ObservableSource<ResponseData<RegisterBean>>>() {
            @Override
            public ObservableSource<ResponseData<RegisterBean>> apply(String s) throws Exception {
                return RetrofitUtils.getInstance(mContext).getApiService().getVerifyCode(ReqBody.getReqString(map));
            }
        } , new BaseObserver<ResponseData<RegisterBean>>() {
            @Override
            protected void onSuccees(ResponseData<RegisterBean> objectResponseData) {
                if(objectResponseData.getStatus() == 0){
                    if(objectResponseData.getData().isIs_register()){
                        register =  objectResponseData.getData().isIs_register();
//                    mLlLogin.setEnabled(true);
            ...
            protected void onFailure(Throwable e, boolean isNetWorkError) {
                if(isNetWorkError)ToastUtil.showToast(getString(R.string.string_request_fails));
            }
        });
    }
        ...
        RetrofitUtils.getInstance(mContext).sendRequset(new Function<String, ObservableSource<ResponseData<UserInfoBean>>>() {
            ...
            protected void onSuccees(ResponseData<UserInfoBean> objectResponseData) {
                if(objectResponseData.getStatus()==0){
                    ...
                    nativeCallRnBean.setOption(Constants.OPTION_LOGIN);
                    ...
                    XGPushManager.registerPush(MainApplication.getInstance(), new XGIOperateCallback() {
                        @Override
